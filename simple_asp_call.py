...
def create_parser():
    description= """
        Provides functionality to run simple ASP calls with a simple interface"""
    parser = argparse.ArgumentParser(
        prog='simple_asp_call',
        description=description,
        epilog='See git repository readme for more details.')

    call_options = CALLS
    parser.add_argument('--call', '-q', type=str, choices=call_options,
        help='What call do you wish to make?')
    recipe_options = RECIPES
    parser.add_argument('--recipe', '-r', type=str, choices=recipe_options,
        help='Which recipe would you like to call?')
